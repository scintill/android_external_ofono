...
static int validate_ussd_data(const struct qmi_ussd_data *data, int size)
{
	...
	if (data->dcs < USS_DCS_ASCII || data->dcs > USS_DCS_UCS2)
		return 1;
...
static void async_orig_ind(struct qmi_result *result, void *user_data)
{
	...
	case 0:
		break;
	case 92:
		qmi_result_get_uint16(result, QMI_VOICE_PARAM_ASYNC_USSD_FAILURE_CASE, &error);
	...
	qmi_ussd = qmi_result_get(result, QMI_VOICE_PARAM_ASYNC_USSD_DATA, &len);
	if (qmi_ussd == NULL)
		return;
	...
	ofono_ussd_notify(ussd, OFONO_USSD_STATUS_NOTIFY, gsm_dcs, qmi_ussd->data, qmi_ussd->length);
	return;
...
/*
 * The cb is called when the request (on modem layer) reports success or failure.
 * It doesn't contain a network result. We get the network answer via VOICE_IND.
 */
			...
			ofono_ussd_cb_t cb, void *data)
{
	 ...
	 * DCS_8BIT and DCS_UCS2 is broken, because the modem firmware (at least on a EC20)
	 * encodes those in-correctly onto the air interface, resulting in wrong decoded
	 * USSD data.
	 */
	qmi_ussd = alloca(sizeof(struct qmi_ussd_data) + utf8_len);
	qmi_ussd->dcs = USS_DCS_ASCII;
	qmi_ussd->length = len;
